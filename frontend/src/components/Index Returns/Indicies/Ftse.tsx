...
function Ftse() {
  const [ftse, setFtse] = useState<any>();

  useEffect(() => {
    const loadFtse = async () => {
      const ftse = await fetch(
        `https://financialmodelingprep.com/api/v3/profile/VGK?apikey=${process.env.REACT_APP_FMP_API_KEY}`
      )
        .then((res) => res.json())
        .then((data) => data);
      console.log(ftse);
  ...
  return (
    <>
      <div className="flex flex-col basis-8 p-2 border border-gray-300 rounded-lg whitespace-nowrap w-full text-sm">
        <div className="font-bold">FTSE</div>
